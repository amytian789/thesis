...
a \texttt{AL\_}). The actual active learning algorithm engine, which is called 
by the simulation engine, may be found in 
...
The active learning algorithm engine is called by the simulation engine 
(Section~\ref{sec:appendicies:al:simulations:simengine}). It calls the various 
active learning methods described in Section~\ref{sec:al:methods}.
...
summarization engine computes the vector $d = \text{diff}(\hat{G}^1,\hat{G}^2)$ 
by utilizing the various 
...
Refer to Section~\ref{sec:usage:newanalysis}.
